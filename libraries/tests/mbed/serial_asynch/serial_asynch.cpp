...
#define TEST_FIRST_TX_PIN PTC17
#define TEST_FIRST_RX_PIN PTC16
#define TEST_SECOND_TX_PIN PTD3
#define TEST_SECOND_RX_PIN PTD2
...
TEST_GROUP(Serial_Asynchronous)
{
    ...
    void setup()
    {
        serial_one = new Serial(TEST_FIRST_TX_PIN, TEST_FIRST_RX_PIN);
        serial_two = new Serial(TEST_SECOND_TX_PIN, TEST_SECOND_RX_PIN);
        tx_complete = false;
        tx_event_flag = 0;
        rx_complete = false;
        rx_event_flag = 0;
...
{
    int rc;
    rc = serial_one->write((void *)tx_buf, SHORT_XFR, -1, cb_tx_done);
    CHECK_EQUAL(0, rc);
...
{
    int rc;
    serial_two->read((void *)rx_buf, SHORT_XFR, -1, cb_rx_done);
    rc = serial_one->write((void *)tx_buf, SHORT_XFR, -1, cb_tx_done);
    CHECK_EQUAL(0, rc);
...
{
    int rc;
    serial_two->read((void *)rx_buf, LONG_XFR, -1, cb_rx_done);
    rc = serial_one->write((void *)tx_buf, LONG_XFR, -1, cb_tx_done);
    CHECK_EQUAL(0, rc);
